...
from .ppo_utils import dump_layernorm, get_rewards_from_function, get_rewards_from_server, replace_model, restore_layernorm
...
class CustomPPOTrainer(PPOTrainer, Trainer):
    ...
    def __init__(
        ...
        additional_eos_token_ids = getattr(self.tokenizer, "additional_special_tokens_ids", None)
        if additional_eos_token_ids is None:
            additional_special_tokens = getattr(self.tokenizer, "additional_special_tokens", []) or []
            additional_eos_token_ids = self.tokenizer.convert_tokens_to_ids(additional_special_tokens)
            if isinstance(additional_eos_token_ids, int):
                additional_eos_token_ids = [additional_eos_token_ids]
            unk_token_id = getattr(self.tokenizer, "unk_token_id", None)
            additional_eos_token_ids = [
                token_id
                for token_id in additional_eos_token_ids
                if token_id is not None and token_id != unk_token_id
            ]

        self.generation_config = GenerationConfig(
            pad_token_id=self.tokenizer.pad_token_id,
            eos_token_id=[self.tokenizer.eos_token_id] + additional_eos_token_ids,
    ...
    def ppo_train(self, resume_from_checkpoint: Optional[str] = None) -> None:
        ...
        for step in tqdm(range(max_steps), disable=not self.is_local_process_zero()):
            ...
            if (step + 1) % self.args.save_steps == 0:  # save checkpoint
                ...
                self._rotate_saved_checkpoints()
    ...
    def get_rewards(
        ...
        if self.finetuning_args.reward_model_type == "api":
            ...
            return get_rewards_from_function(self.reward_model, response_texts)
    ...
    def _rotate_saved_checkpoints(self) -> None:
        r"""Remove older PPO checkpoints after manual save_model calls."""
        save_total_limit = self.args.save_total_limit
        if not save_total_limit or save_total_limit <= 0 or not self.args.should_save:
            return
        ...
        for name in os.listdir(self.args.output_dir):
            ...
            if match is not None:
                checkpoints.append((int(match.group(1)), os.path.join(self.args.output_dir, name)))

        checkpoints.sort(key=lambda item: item[0])
        for _, checkpoint in checkpoints[:-save_total_limit]:
            logger.info_rank0(f"Deleting older checkpoint [{checkpoint}] due to args.save_total_limit")
