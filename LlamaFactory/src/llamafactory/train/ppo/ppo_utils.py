...
def _score_code_response(text: str) -> float:
    text = str(text or "").strip()
...
def get_rewards_from_function(function_name: str | None, responses: list[str]) -> "torch.Tensor":
    r"""Get reward scores from a local reward function."""
    if function_name not in [None, "", "code_heuristic"]:
        raise ValueError(f"Unknown reward function: {function_name}")

    rewards = [_score_code_response(response) for response in responses]
